...
import { formatMoney, formatMonthShort, formatYearMonthCompact } from '../api/adapters'
import { SpendBarChart, type SpendBucket } from './SpendBarChart'
import { BUDGET_GROUP_COLORS, EmptyState, Skeleton, SectionLabel, t, fonts } from '../ui'
...
export default function BudgetInsights({
  ...
  const chartMonths = useMemo(() => {
    const withSpendOrPeriod = months.filter(m =>
  ...
  const avgSpend = chartMonths.length > 0
    ? chartMonths.reduce((sum, m) => sum + m.outflowActual, 0) / chartMonths.length
    : 0
  ...
  const vsPrevious = useMemo(() => {
    ...
    const buildDeltas = (currentMap: Map<string, SpendRow>, previousMap: Map<string, SpendRow>) => {
    ...
    let rows = buildDeltas(collectOutflowLines(selected), collectOutflowLines(previous))
    if (rows.length === 0) {
      rows = buildDeltas(collectOutflowGroups(selected), collectOutflowGroups(previous))
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingBottom: 24 }}>
      {snapshot && (
        ...
        >
          <SnapshotStat
            ...
            valueColor={snapshot.spent > snapshot.budgeted + 0.005 ? t.attention : t.text}
          />
          <SnapshotStat
            label="Net position"
            value={formatMoney(Math.abs(snapshot.net), currency)}
            detail={snapshot.net >= 0 ? 'surplus' : 'over'}
            valueColor={snapshot.net >= 0 ? t.success : t.attention}
            divided
          />
          <SnapshotStat
            label="Days remaining"
            value={`${snapshot.days.remaining}`}
            detail={`of ${snapshot.days.total} in cycle`}
            divided
          />
        </section>
      )}
                ...
                >
                  ...
                  <div style={{
                    ...
                    <span style={{
                      fontSize: 11,
                      fontWeight: 500,
                      color: over ? t.attention : under ? t.success : t.textTer,
                      fontFamily: fonts.ui,
                      fontVariantNumeric: 'tabular-nums',
                    }}>
                      {over || under ? signedMoney(row.variance, currency) : 'on plan'}
                    </span>
                  </div>
                </li>
              )
      ...
      {vsPrevious && (
        <section>
          <SectionLabel>
            Vs previous cycle · {formatYearMonthCompact(vsPrevious.previousMonth)}
          </SectionLabel>
          <ul
                ...
                >
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{
                      ...
                      {formatMoney(row.previous, currency)} → {formatMoney(row.current, currency)}
                    </div>
                  </div>
                  <span style={{
                    fontSize: 13,
