...
function partsInTimezone(iso: string, timeZone: string): { date: string; time: string } {
...
export function toCalendarEvent(ev: EventOut, timeZone = 'UTC'): CalendarEvent {
  ...
  return {
    ...
    reminder: ev.reminder_minutes?.[0] != null ? String(ev.reminder_minutes[0]) : undefined,
    repeat: ev.recurrence_rule ?? undefined,
  }
}

function dueLabel(dueAt: string | null, today: string, timeZone: string): string {
...
function entityToScreen(entityType: string | null): Screen | undefined {
  ...
  if (entityType === 'shopping' || entityType === 'shopping_item') return 'shopping'
...
export function toNotification(n: NotificationOut): Notification {
  const type =
    n.type === 'calendar' || n.type === 'task' || n.type === 'shopping' || n.type === 'family'
      ? n.type
      : 'family'
  return {
    id: n.id,
